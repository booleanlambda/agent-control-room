...
async function author(job,sources,authorModel) {
  const sourceData=sources.map(x=>({url:x.url,source_title:x.source_title,publisher:x.publisher,
    coverage:x.coverage,excerpt:x.excerpt.slice(0,3000)}));
...
async function review(job,sources,publicSpec,privateAssessment,reviewerModel) {
  ...
  return call(reviewerModel,verifier,
    JSON.stringify({domain:job.domain,official_sources:sources.map(x=>({url:x.url,source_title:x.source_title,publisher:x.publisher,excerpt:x.excerpt.slice(0,2500)})),
      public_spec:publicSpec,private_assessment:privateAssessment}).slice(0,44000)+
...
async function processJob(job) {
  let authorModel=process.env.AAU_STANDARD_AUTHOR_MODEL||'mistralai/mistral-nemotron';
  ...
  const reviewed=await review(job,sources,publicSpec,privateAssessment,reviewerModel);
