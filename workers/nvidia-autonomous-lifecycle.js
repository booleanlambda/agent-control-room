...
import { runNvidiaWake } from './nvidia-single-agent-wake.js';
...
const MAIN_QUEUE = 'aau.wake';
const ARM_POLL_MS = Math.max(1000, Number(process.env.AAU_AUTONOMOUS_WAKE_ARM_POLL_MS || 3000));
const HEARTBEAT_MS = Math.max(10000, Number(process.env.AAU_AUTONOMOUS_WAKE_HEARTBEAT_MS || 30000));
...
const workerId = String(process.env.AAU_AUTONOMOUS_WAKE_WORKER_ID || `render:nvidia-lifecycle:${process.env.RENDER_INSTANCE_ID || process.pid}`).trim();
...
function delayQueueName(wakeRequestId) {
...
async function publishArmedWake(channel, row) {
  ...
  if (delayMs > 1000) {
    delayQueue = delayQueueName(row.wake_request_id);
...
async function heartbeat() {
  try {
    ...
    console.log('AAU_AUTONOMOUS_WAKE_HEARTBEAT', JSON.stringify(result));
  } catch (error) {
    console.error('AAU_AUTONOMOUS_WAKE_HEARTBEAT_FAILED', String(error?.message || error));
  }
}

async function handleWake(channel, msg) {
  if (!msg) return;
  let event = null;
  try { event = JSON.parse(msg.content.toString('utf8')); } catch {}

  if (!event || event.type !== 'autonomous_wake' || !event.wake_request_id || !event.agent_id) {
    console.warn('AAU_AUTONOMOUS_WAKE_UNSUPPORTED_MESSAGE', JSON.stringify({
      message_id: msg.properties?.messageId || null,
      type: event?.type || msg.properties?.type || null,
    }));
    channel.ack(msg);
    return;
  }
  ...
  try {
    const report = await runNvidiaWake({
      wakeRequestId: event.wake_request_id,
    ...
    channel.ack(msg);
    console.log('AAU_AUTONOMOUS_WAKE_CONSUMED', JSON.stringify({
      wake_request_id: event.wake_request_id,
      agent_id: event.agent_id,
      selected_action: report?.selected_action || null,
      next_wakes: report?.next_wakes || [],
    }));
  } catch (error) {
    const message = String(error?.message || error);
    if (!message.includes('wake_not_claimable')) {
      await rpc('aau_bridge_reset_autonomous_wake_arm', {
        p_wake_request_id: event.wake_request_id,
        p_worker_id: workerId,
    ...
    if (event.delay_queue) await channel.deleteQueue(event.delay_queue).catch(() => {});
    channel.ack(msg);
    console.error('AAU_AUTONOMOUS_WAKE_CONSUME_FAILED', event.wake_request_id, message);
  }
}
...
export async function startNvidiaAutonomousLifecycle() {
  ...
  connection.on('error', (error) => console.error('AAU_AUTONOMOUS_WAKE_RABBIT_ERROR', String(error?.message || error)));
  connection.on('close', () => console.error('AAU_AUTONOMOUS_WAKE_RABBIT_CLOSED'));

  await heartbeat();
  await armPending(channel);

  await channel.consume(MAIN_QUEUE, (msg) => {
    void handleWake(channel, msg);
  }, { noAck: false });

  console.log('AAU_AUTONOMOUS_WAKE_CONSUMER_BOUND', JSON.stringify({ queue: MAIN_QUEUE, worker_id: workerId }));

  const armTimer = setInterval(() => void armPending(channel).catch((e) => console.error('AAU_AUTONOMOUS_WAKE_ARM_SCAN_FAILED', String(e?.message || e))), ARM_POLL_MS);
  ...
  return { ok: true, queue: MAIN_QUEUE, worker_id: workerId };
}
