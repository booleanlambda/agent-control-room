...
### Deployment and Julian pilot record

- Database storage: `sql/aau-intervention-independent-assessment-v0.2.sql`, Git commit `155c7bfea528e094322ca0b7c370716f259e67c7`, applied to Supabase. Review-close guard: `sql/aau-intervention-review-closure-v0.2.sql` plus `sql/aau-intervention-review-close-delivery-v0.2.1.sql`, applied. The guard requires two completed reviewer records from distinct actual models/invocations and the frozen evidence hash, a responding feedback message and referenced outcome evidence before closing. The records reside in a private RLS-enabled `agent_lab.intervention_review_cases` table; no permission or official verdict is changed.
- Julian EFRA intervention case: `9eacf264-408a-4cb7-afd8-a6f2f2dbd8df`, scoped to work item `527020ee-222d-465b-ba96-24850c38e149` and v1.1 file `011e82a3-a72c-47f2-b3fd-23688f74d6b4`, content SHA-256 `62bc61eb12ab5afc7a4c48a27796b2a2c18d8357090ef22b551109785c8672c3`. It links the operator T1/T4 execution report; this report is **not** a model-authenticator or model-adjudicator assessment.
- First readiness query: `frozen_evidence_matches=true`, `authenticator_assessed=false`, `adjudicator_assessed=false`, `feedback_delivered=false`, `outcome_evidence_present=false`. Case remains `review_pending`. Earlier reviewer endpoint timeouts are historical diagnostics, **not** claimed invocation attempts on this case.
- Scope limit: no generalized reviewer-dispatch worker or automatic feedback delivery has been connected yet. These are explicit next integration requirements; a stored pending case and installed close guard must not be represented as a completed model assessment.


## v0.3 explicit user-authorized manual A/A mode (2026-09-20)

**User directive:** When the operator invokes manual intervention, ChatGPT must perform the authenticator and adjudicator roles directly in that response instead of leaving a newly opened case awaiting unreliable A/A endpoints. Run both substantive passes immediately against the frozen evidence where available, cite saved file IDs and test logs, and provide the agent actionable findings through the established admin-chat path. This instruction overrides the v0.2 *workflow expectation* to wait for automated reviewer calls, but it does not override independent-verification requirements for an official expertise/product verdict.

- The **manual authenticator pass** checks primary evidence, file hashes, test execution, criteria, unsupported claims, and reproducibility. The **manual adjudicator pass** challenges the initial assessment using original evidence, alternative explanations, adverse controls and limitations. Each pass is a separately written substantive assessment with its own reference and corrections.
- When both passes are by ChatGPT GPT-5.6 Sol, label the result **same-model manual dual-role** and set `independent_model_review=false`. Two passes by one model are not independent model/provider invocations. Never claim Moonshot, Meta or NVIDIA was called if not actually invoked.
- Preserve original evidence and distinguish a previous test from a subsequently revised file. Agent correction, feedback dispatch, delivery, post-feedback outcomes and official verdicts are separately recorded.
- **Manual review case lifecycle:** `manual_assessed` after both real passes, `manual_feedback_sent` after feedback dispatch, `manual_review_complete` only after feedback delivery is confirmed and evidence-backed follow-up exists. This is distinct from the ordinary `closed` independent-review route and cannot be used to grant expertise, thesis acceptance, product conformance or privileges. When delivery is still pending, report that fact while the two assessments remain completed; do not mislabel it "waiting for authentication."
- The implementation is `sql/aau-intervention-manual-dual-pass-v0.3.sql`, an explicitly scoped schema extension and guard for manual closure. The v0.2 independent-review guard remains in force for ordinary `closed`; the user-authorized manual path must carry truthful reviewer identity and lack-of-independence metadata.
- If no execution capability or authoritative evidence exists, perform the two **assessment** passes anyway, mark the untestable criterion `BLOCKED`, and tell the agent exactly what independent evidence is missing. Do not invent a test or let a reviewer outage stall unrelated reversible repairs.

**First application:** Julian EFRA case `9eacf264-408a-4cb7-afd8-a6f2f2dbd8df`. Manual dual-pass report: [AAU_EFRA_MANUAL_DUAL_REVIEW_2026-09-20.md](./AAU_EFRA_MANUAL_DUAL_REVIEW_2026-09-20.md). Matching v2 T1/T4 source independently replayed in isolated Python and 13 operator-targeted checks passed; v1.2 provenance incorrectly cites 12/13 from older defective fixtures. Both ChatGPT passes are stored as `manual_complete`; feedback message `dedcab8a-8e5f-4e67-9a24-e24b889cbfa0` was dispatched. Official expertise run `7d2a052f-c1ae-4567-90cf-626f442a3457` remains held, without grade.
