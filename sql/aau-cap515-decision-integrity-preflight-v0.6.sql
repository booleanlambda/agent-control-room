...
begin
  ...
  elsif coalesce(v_canon#>>'{core_economics,unit_economics,discount_rate}','') ~ '^[0-9]+([.][0-9]+)?$' then
    v_discount:=(v_canon#>>'{core_economics,unit_economics,discount_rate}')::numeric;
  ...
  elsif coalesce(v_canon#>>'{core_economics,unit_economics,ltv_calculation,ltv_value}','') ~ '^[0-9]+([.][0-9]+)?$' then
    v_canon_ltv:=(v_canon#>>'{core_economics,unit_economics,ltv_calculation,ltv_value}')::numeric;
  end if;
  if coalesce(v_canon#>>'{unit_economics,ltv_cac_ratio}','') ~ '^[0-9]+([.][0-9]+)?$' then
    v_canon_ratio:=(v_canon#>>'{unit_economics,ltv_cac_ratio}')::numeric;
  elsif coalesce(v_canon#>>'{core_economics,unit_economics,ltv_calculation,ltv_cac_ratio}','') ~ '^[0-9]+([.][0-9]+)?$' then
    v_canon_ratio:=(v_canon#>>'{core_economics,unit_economics,ltv_calculation,ltv_cac_ratio}')::numeric;
  end if;
  ...
  -- Validate current canonical churn-sensitivity arithmetic against its own stated formula.
  if coalesce(v_canon#>>'{core_economics,unit_economics,ltv_calculation,annual_margin}','') ~ '^[0-9]+([.][0-9]+)?$' then
    v_margin:=(v_canon#>>'{core_economics,unit_economics,ltv_calculation,annual_margin}')::numeric;
  end if;
  if coalesce(v_canon#>>'{core_economics,customer_acquisition,cac}','') ~ '^[0-9]+([.][0-9]+)?$' then
    v_cac:=(v_canon#>>'{core_economics,customer_acquisition,cac}')::numeric;
  end if;

  if v_margin is not null and v_cac is not null and v_cac>0 and v_discount>0
     and jsonb_typeof(v_canon#>'{sensitivity_analysis,churn_sensitivity}')='object' then
    select count(*) into v_sensitivity_errors
    from jsonb_each(v_canon#>'{sensitivity_analysis,churn_sensitivity}') kv
    cross join lateral regexp_match(kv.key,'^([0-9]+)_percent_churn$') m
    where coalesce(kv.value->>'ltv','') ~ '^[0-9]+([.][0-9]+)?$'
      and coalesce(kv.value->>'ltv_cac','') ~ '^[0-9]+([.][0-9]+)?$'
      and (
        abs((kv.value->>'ltv')::numeric - (v_margin / ((m[1])::numeric/100 + v_discount))) > 1
        or abs((kv.value->>'ltv_cac')::numeric - ((v_margin / ((m[1])::numeric/100 + v_discount))/v_cac)) > 0.02
      );
  ...
  -- BUILD may override a failed kill criterion only when the override is explicit and auditable.
  if upper(coalesce(v_board->>'final_decision',''))='BUILD'
     and jsonb_typeof(v_board#>'{kill_criteria_evaluation,criteria}')='array' then
    ...
    if v_failed_kill_criteria>0
       and nullif(btrim(coalesce(v_board->>'kill_criterion_override_rationale','')),'') is null then
  ...
  return jsonb_build_object(
    'version','cap515_reconciliation_preflight_v0_6_decision_integrity',
    ...
    'rule','Minimum integrity gate. Supports current/legacy schemas and receipt-backed evidence, validates canonical churn-sensitivity arithmetic, and requires an explicit auditable override when BUILD conflicts with a failed kill criterion. Passing does not award CAP515 or release the assessment hold.'
  );
end;
$function$;
