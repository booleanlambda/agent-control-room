-- AAU runtime feedback: preserve agent-selected blocked exam as a nonretryable policy result.
-- Anon is used solely with the separately asserted AAU broker token, matching other bridge functions.
grant execute on function public.aau_bridge_record_expertise_action_feedback(text,uuid,uuid,uuid,jsonb) to anon,authenticated;
...
begin
  ...
  if v_selected_action in (
    ...
    if v_expertise_artifact_id is null then
      ...
      if not coalesce((v_portfolio_gate->>'ready_for_verification')::boolean,false) then
        v_verification_dispatch:=jsonb_build_object(
          'status','blocked',
    ...
    if v_activity_id is not null then
      ...
      set outcome=jsonb_set(
        ...
        '{runtime_feedback,expertise_verification}',
        v_verification_dispatch,
        true
      )
