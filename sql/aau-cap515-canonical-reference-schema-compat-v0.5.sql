...
begin
  ...
  if coalesce(v_canon#>>'{unit_economics,discount_rate}','') ~ '^[0-9]+([.][0-9]+)?
  ...
  if coalesce(v_canon#>>'{unit_economics,ltv_calculated}','') ~ '^[0-9]+([.][0-9]+)?
  ...
  return jsonb_build_object(
    'version','cap515_reconciliation_preflight_v0_5_canonical_reference_compat',
    ...
    'rule','Minimum mechanical/evidence integrity only. Supports canonical_model_reference or canonical_reference, legacy/current canonical financial schemas, and claims[] evidence. External sources count only when exact URL (and SHA when supplied) matches a persisted fetched-text research receipt. Passing does not award CAP515 or release the assessment hold.'
  );
end;
$function$;
...
COMMIT;
 then
  ...
  elsif coalesce(v_canon#>>'{core_economics,unit_economics,discount_rate}','') ~ '^[0-9]+([.][0-9]+)?
...
COMMIT;
 then
  ...
  elsif coalesce(v_fin#>>'{canonical_inputs,unit_economics,discount_rate}','') ~ '^[0-9]+([.][0-9]+)?
...
COMMIT;
 then
    v_canon_ltv:=(v_canon#>>'{unit_economics,ltv_calculated}')::numeric;
  elsif coalesce(v_canon#>>'{core_economics,unit_economics,ltv_calculation,ltv_value}','') ~ '^[0-9]+([.][0-9]+)?
...
COMMIT;
 then
  ...
  if coalesce(v_canon#>>'{unit_economics,ltv_cac_ratio}','') ~ '^[0-9]+([.][0-9]+)?
...
COMMIT;
 then
    v_canon_ratio:=(v_canon#>>'{unit_economics,ltv_cac_ratio}')::numeric;
  elsif coalesce(v_canon#>>'{core_economics,unit_economics,ltv_calculation,ltv_cac_ratio}','') ~ '^[0-9]+([.][0-9]+)?
