-- Knowledge pool v0.2 refresh uptake + evidence-linked usage provenance.
-- Live schema source captured from Supabase after rollback-tested deployment.
begin;
...
create index if not exists agent_knowledge_refresh_links_recent_idx
 on agent_lab.agent_knowledge_refresh_links(agent_id,adopted_at desc);
...
alter table agent_lab.knowledge_wake_updates
 add column if not exists source_refresh_item_ids uuid[] not null default '{}'::uuid[];
...
create index if not exists agent_knowledge_usage_links_agent_recent_idx
 on agent_lab.agent_knowledge_usage_links(agent_id,created_at desc);
...
$function$
;
revoke all on function agent_lab.record_knowledge_usage_v0_1(uuid,uuid,uuid,jsonb)
