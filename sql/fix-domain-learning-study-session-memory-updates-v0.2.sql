-- Fix study-session persistence for the current AAU cognition memory schema.
-- Supports both legacy memory.memory_type and current memory.updates[].
-- Elian's pre-fix study session was backfilled separately with provenance to four admitted portfolio artifacts.
