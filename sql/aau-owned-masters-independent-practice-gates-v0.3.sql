...
begin
  ...
  if coalesce((v_art.metadata->>'standards_required')::boolean,false)
     and coalesce((v_academic->>'ready')::boolean,false) then
    v_practice:=agent_lab.evaluate_owned_master_practice_v0_3(p_agent_id,v_art.expertise_artifact_id);
  elsif not coalesce((v_art.metadata->>'standards_required')::boolean,false) then
    v_practice:=jsonb_build_object('ready',true,'reason','historical_artifact_not_v0_3_bound');
  else
    v_practice:=jsonb_build_object('ready',false,'reason','academic_standard_not_approved');
  end if;
  ...
  return jsonb_build_object(
    ...
    'verification_request_allowed',
      ...
      and coalesce((v_practice->>'ready')::boolean,false),
    'academic_standard_gate',v_academic,
    'academic_practice_gate',v_practice,
