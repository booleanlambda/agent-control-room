...
-- AAU Complex Work evidence reconciliation v0.2 (scoped only to opted-in pilot work).
-- This is evidence registration, NOT validation or automatic completion.
...
END;
$reconcile$;
REVOKE ALL ON FUNCTION agent_lab.reconcile_complex_work_file_v0_2(uuid)
  FROM PUBLIC,anon,authenticated;

CREATE OR REPLACE FUNCTION agent_lab.after_agent_file_reconcile_complex_work_v0_2()
RETURNS trigger LANGUAGE plpgsql SECURITY INVOKER
SET search_path TO 'pg_catalog','agent_lab'
AS $trigger$
BEGIN
 IF NEW.agent_id='3b190e7e-b452-4888-9850-3a35a4e95cad'::uuid THEN
   PERFORM agent_lab.reconcile_complex_work_file_v0_2(NEW.file_id);
 END IF;
 RETURN NEW;
END;
$trigger$;
REVOKE ALL ON FUNCTION agent_lab.after_agent_file_reconcile_complex_work_v0_2()
 FROM PUBLIC,anon,authenticated;
DROP TRIGGER IF EXISTS aau_complex_work_pilot_file_after_insert_v0_2 ON agent_lab.agent_files;
CREATE TRIGGER aau_complex_work_pilot_file_after_insert_v0_2
AFTER INSERT ON agent_lab.agent_files
FOR EACH ROW EXECUTE FUNCTION agent_lab.after_agent_file_reconcile_complex_work_v0_2();
