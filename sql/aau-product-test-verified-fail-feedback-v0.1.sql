-- AAU independent product testing: preserve verified failures and return actionable retry feedback.
-- Applied September 20, 2026. No product verdict, grant or agent state is reset.
...
begin
 ...
 if v_run.status='verified_fail' and not v_discovery_gap then
   return v_eligibility||jsonb_build_object(
     'ok',false,'status','verified_fail','disposition','existing_verified_fail',
     'retry_requested',false,'reason','verified_load_or_functional_failure_requires_agent_remediation',
     'previous_run_id',v_run.product_test_run_id,
     'load_metrics',coalesce(v_run.metrics->'load','{}'::jsonb),
     'failed_gates',coalesce((
        select jsonb_agg(elem) from jsonb_array_elements(coalesce(v_run.final_report->'gate_results','[]'::jsonb)) elem
        where elem->>'status'<>'PASS'
      ),'[]'::jsonb),
     'failed_adversarial',coalesce((
        select jsonb_agg(elem) from jsonb_array_elements(coalesce(v_run.final_report->'adversarial_results','[]'::jsonb)) elem
        where elem->>'status'<>'PASS'
      ),'[]'::jsonb),
     'load_test_result',coalesce(v_run.final_report->'load_test_result','{}'::jsonb),
     'next_step','Analyze the recorded failure and implement a material agent-authored change before a new canonical review and test. The runtime must not reset or reapprove this verdict.'
   );
 end if;
 ...
 -- A single additional operator remediation is permitted only for JSON planner
 -- exhaustion after deploying the bounded syntax repair, never for a real verdict.
 if v_generation>=2 and not (
   v_generation=2 and p_activity_id is null
   and v_run.status='failed'
   and v_run.error_code='product_test_executor_error'
   and coalesce(v_run.error_message,'') like 'product_test_planner_models_exhausted:%'
 ) then
   return v_eligibility||jsonb_build_object('ok',false,'status','blocked',
