  ...
  try {
    if (req.method === 'GET') {
      return res.status(200).json({
        ok: true,
        runtime: 'vercel_node_serverless_ai_gateway_oidc',
        model: MODEL,
    ...
    if (action === 'packet') {
      ...
      return res.status(200).json({ ok: true, packet, usage: out.usage });
    }

    if (action === 'grade') {
      const packet = req.body?.packet;
      const answers = req.body?.answers;
      if (!packet || !Array.isArray(answers) || answers.length !== 8) {
        return res.status(400).json({ error: 'packet_and_exactly_8_answers_required' });
      }
      const expectedPacketHash = packet.packet_sha256;
      const clone = { ...packet };
      delete clone.packet_sha256;
      const recomputed = sha256(clone);
      if (expectedPacketHash !== recomputed) return res.status(409).json({ error: 'packet_hash_mismatch', recomputed });
      ...
      return res.status(200).json({ ok: true, grade, usage: out.usage });
    }
